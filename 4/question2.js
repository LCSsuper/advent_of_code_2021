...
for (const number of numbersToCall) {
    for (let i = 0; i < cards.length; i++) {
        ...
        markCalledNumber(cards[i], number);
        if (cardHasBingo(cards[i])) {
            const remainingCards = cards.filter((card) => card !== null);
            if (remainingCards.length === 1) {
                const [lastCard] = remainingCards;
                const sumOfUnmarkedNumbers = addRemainingNumbers(lastCard);
